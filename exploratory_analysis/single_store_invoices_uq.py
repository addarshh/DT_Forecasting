...
y_in_samp_preds = pipeline_obj.ensem_obj.main_methods_df_in.values
...
in_samp_diffs_squared = [(y_in_samp_preds[:,indez_f]-pipeline_obj.model_data.y_train)**2 for indez_f in range(num_methods)]
...
for j in range(15):
    ...
    for i in range(samp_size):
        cur_weighted_sum_array = cur_weights*norm.pdf(pipeline_obj.model_data.y_train[i],y_in_samp_preds[i, :], np.sqrt(cur_sigma_2))
...
num_mc_samples = 1000
y_out_samp_preds = pipeline_obj.ensem_obj.main_methods_df_out.values
...
mse = np.round(np.mean((out_samp_preds-pipeline_obj.model_data.y_test)**2),2)
corr = np.round(np.corrcoef(out_samp_preds,pipeline_obj.model_data.y_test)[0,1],2)
plt.title(
    f'BMA Single Store Model; Week Ahead Forecast for Store: {cur_store} \n Out-of-sample MSE: {mse} Corr: {corr}',
    fontsize=20)
